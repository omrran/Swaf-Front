<template>
  <div class="pa-5">
    ...
    <v-btn depressed color="primary" class="mx-1" @click="updateOrder">
      Update
    </v-btn>
    <v-btn
      depressed
...
export default {
  ...
  methods: {
    updateOrder() {
      ...
      Axios.put(`http://127.0.0.1:8000/api/orders/${this.$route.params.id}`, {
  ...
  created() {
    //get the order that i will update it
    Axios.get(`http://127.0.0.1:8000/api/orders/${this.$route.params.id}`).then(
      (resp) => {
        console.log("current Order :", resp.data);
        this.CustomerId = resp.data.CustomerId;
        this.OrderNumber = resp.data.OrderNumber;
        this.TotalAmount = resp.data.TotalAmount;
        this.OrderDateDate = resp.data.OrderDate.split(" ")[0];
        this.OrderDateTime = resp.data.OrderDate.split(" ")[1];
      }
    );
    //get all orderItems that ralated with current order
    Axios.get(
      `http://127.0.0.1:8000/api/orders/get-order-items/${this.$route.params.id}`
    ).then((resp) => {
      console.log("Order items :", resp.data);
